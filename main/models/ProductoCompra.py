...
class ProductoCompra(db.Model):
    ...
    producto = db.relationship('Producto', back_populates="productocompra", uselist=False, single_parent=True)
    compraId = db.Column(db.Integer, db.ForeignKey('compra.id'), nullable=False)
    compra = db.relationship('Compra', back_populates="productocompra", uselist=False, single_parent=True)


    def __repr__(self):
        return f'producto: {self.producto.to_json()}'
